...
import {updateCustomer} from "@/lib/actions/admin/customer.action";
...
function AddEditCustomer({ customer, onNeedRefresh}: Props) {
    ...
    const onSubmit = async (values: z.infer<typeof CustomerValidation>) => {
        try {
            ...
            await updateCustomer({
                id: customer?.id === undefined || customer?.id === null ? "" : customer?.id,
                icon: values.logo,
                title: values.title,
                url: values.url
            })
