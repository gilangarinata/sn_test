...
function WhySolarTable() {

    const [achievements, setAchievements] = useState<WhySolar[]>()
    async function getAchievements() {
        const achievements = await fetchWhySolar()
    ...
    const [open, setOpen] = useState<{banner : WhySolar | null, isOpen : boolean}>({banner: null, isOpen:false});
    const [deleteLoading, setDeleteLoading] = useState(false);
    const [createBannerOpen, setCreateBannerOpen] = useState<{banner : WhySolar | null, isOpen : boolean}>({banner: null, isOpen: false})
    ...
    return (
            <div className="flex flex-col">
                ...
                <Dialog open={createBannerOpen.isOpen} onOpenChange={(isOpen) => setCreateBannerOpen(prevState => {
                    ...
                    }} variant="outline" className="w-fit ml-8"><PlusIcon className="w-4 h-4"/> Add Why Solar</Button>
                    <DialogContent className="w-8">
                        <DialogHeader>
                            <DialogTitle>{"Add new banner"}</DialogTitle>
                        </DialogHeader>
                        <DialogBody className="overflow-y-auto max-h-[420px]">
                            <AddEditWhySolar achievement={createBannerOpen.banner == null ? undefined : createBannerOpen.banner} onNeedRefresh={() => {
