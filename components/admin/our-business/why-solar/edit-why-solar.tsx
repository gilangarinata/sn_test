...
interface Props {
    achievement?: WhySolar;
    onNeedRefresh : () => void
}

function AddEditWhySolar({ achievement, onNeedRefresh}: Props) {
    ...
    const form = useForm<z.infer<typeof WhySolarValidation>>({
        resolver: zodResolver(WhySolarValidation),
        defaultValues: {
    ...
    const onSubmit = async (values: z.infer<typeof WhySolarValidation>) => {
        try {
            setSaveLoading(true)
...
export default AddEditWhySolar;