...
function BannerTable() {
    ...
    const [achievements, setAchievements] = useState<OurBusinessBanner[]>()
    async function getAchievements() {
        const achievements = await fetchOurBusinessBanners()
    ...
    const [open, setOpen] = useState<{banner : OurBusinessBanner | null, isOpen : boolean}>({banner: null, isOpen:false});
    const [deleteLoading, setDeleteLoading] = useState(false);
    const [createBannerOpen, setCreateBannerOpen] = useState<{banner : OurBusinessBanner | null, isOpen : boolean}>({banner: null, isOpen: false})
                ...
                <Dialog open={createBannerOpen.isOpen} onOpenChange={(isOpen) => setCreateBannerOpen(prevState => {
                    ...
                    <DialogContent className="w-8">
                        ...
                        <DialogBody className="overflow-y-auto max-h-[420px]">
                            <AddEditOurBusinessBanner achievement={createBannerOpen.banner == null ? undefined : createBannerOpen.banner} onNeedRefresh={() => {
