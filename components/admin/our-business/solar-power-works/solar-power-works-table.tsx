...
function SolarPowerWorksTable() {

    const [achievements, setAchievements] = useState<SolarPowerWorks[]>()
    async function getAchievements() {
        const achievements = await fetchSolarPowerWorks()
    ...
    const [open, setOpen] = useState<{banner : SolarPowerWorks | null, isOpen : boolean}>({banner: null, isOpen:false});
    const [deleteLoading, setDeleteLoading] = useState(false);
    const [createBannerOpen, setCreateBannerOpen] = useState<{banner : SolarPowerWorks | null, isOpen : boolean}>({banner: null, isOpen: false})
    ...
    return (
            <div className="flex flex-col">
                ...
                <Dialog open={createBannerOpen.isOpen} onOpenChange={(isOpen) => setCreateBannerOpen(prevState => {
                    ...
                    }} variant="outline" className="w-fit ml-8"><PlusIcon className="w-4 h-4"/> Add Item</Button>
                    <DialogContent className="w-8">
                        <DialogHeader>
                            <DialogTitle>{"Add new item"}</DialogTitle>
                        </DialogHeader>
                        <DialogBody className="overflow-y-auto max-h-[420px]">
                            <AddEditSolarPowerWorks achievement={createBannerOpen.banner == null ? undefined : createBannerOpen.banner} onNeedRefresh={() => {
                        ...
                        <TableBody>
                            {achievements?.map((achievement) => (
                                <TableRow key={achievement.id}>
                                    ...
                                    <TableCell>{achievement.subtitle}</TableCell>
                                    <TableCell><p dangerouslySetInnerHTML={{__html:achievement.description}}></p></TableCell>
                                    <TableCell>
...
export default SolarPowerWorksTable;