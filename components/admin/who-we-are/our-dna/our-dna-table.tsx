...
function OurDnaTable() {

    const [achievements, setAchievements] = useState<OurDna[]>()
    async function getAchievements() {
        const achievements = await fetchOurDna()
    ...
    const [open, setOpen] = useState<{banner : OurDna | null, isOpen : boolean}>({banner: null, isOpen:false});
    const [deleteLoading, setDeleteLoading] = useState(false);
    const [createBannerOpen, setCreateBannerOpen] = useState<{banner : OurDna | null, isOpen : boolean}>({banner: null, isOpen: false})
    ...
    return (
            <div className="flex flex-col">
                ...
                <Dialog open={createBannerOpen.isOpen} onOpenChange={(isOpen) => setCreateBannerOpen(prevState => {
                    ...
                    }} variant="outline" className="w-fit ml-8"><PlusIcon className="w-4 h-4"/> Add Our DNA</Button>
                    <DialogContent className="w-8">
                        <DialogHeader>
                            <DialogTitle>{"Add new achievement"}</DialogTitle>
                        </DialogHeader>
                        <DialogBody className="overflow-y-auto max-h-[420px]">
                            <AddEditOurDna achievement={createBannerOpen.banner == null ? undefined : createBannerOpen.banner} onNeedRefresh={() => {
