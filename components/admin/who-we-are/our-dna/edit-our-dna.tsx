...
import {OurDnaValidation} from "@/lib/validations/our-dna-validation";
import {updateOurDna} from "@/lib/actions/admin/our-dna.action";
...
function AddEditOurDna({ achievement, onNeedRefresh}: Props) {
    ...
    const form = useForm<z.infer<typeof OurDnaValidation>>({
        resolver: zodResolver(OurDnaValidation),
        defaultValues: {
    ...
    const onSubmit = async (values: z.infer<typeof OurDnaValidation>) => {
        try {
            setSaveLoading(true)
