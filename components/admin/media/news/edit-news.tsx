...
interface Props {
    achievement?: News; // this is your current News being edited
}

type ContributorRole = "penulis" | "editor";
...
function AddEditNews({ achievement }: Props) {
    ...
    const [penulisId, setPenulisId] = useState<string | null>(null);
    const [editorId, setEditorId] = useState<string | null>(null);
    const [penulisName, setPenulisName] = useState<string>("");
    const [penulisEmail, setPenulisEmail] = useState<string>("");
    ...
    const form = useForm<z.infer<typeof NewsValidation>>({
        ...
        defaultValues: {
            ...
            tags: achievement?.tags?.map((t) => (t as any).tag).join(",") ?? "",
            relatedNews: achievement?.relatedNews?.map((t: any) => t._id).join(",") ?? "",
    ...
    // === Contributors (NEW): load existing from API ===
    useEffect(() => {
        if (!newsObjectId) return;
    ...
    const startUpload = async (logo: File[]): Promise<{ message: string; fileUrl: string }[]> => {
        const file = logo[0];
        const formData = new FormData();
        formData.append("file", file);

        try {
            const response = await axiosInstance.post<{ message: string; fileUrl: string }[]>("/api/upload", formData, {
    ...
    return (
        <Form {...form}>
            <form className="flex flex-col justify-start gap-4 px-4 py-10" onSubmit={form.handleSubmit(onSubmit)}>
                <FormField
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem className="flex w-full flex-col">
                            <FormLabel className="text-base-semibold text-light-2">Recommendation News ID (comma separated)</FormLabel>
                            <FormControl>
                                <Input type="text" className="account-form_input no-focus" {...field} />
                            </FormControl>
                            <FormMessage />
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem className="flex w-full flex-col">
                            <FormLabel className="text-base-semibold text-light-2">Tags / Keywords (comma separated)</FormLabel>
                            <FormControl>
                                <Input type="text" className="account-form_input no-focus" {...field} />
                            </FormControl>
                            <FormMessage />
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="border rounded-lg p-4">
                        <h3 className="font-medium mb-3">Penulis</h3>
                        <div className="space-y-3">
                            <div>
                                <FormLabel>Nama</FormLabel>
                                <Input value={penulisName} onChange={(e) => setPenulisName(e.target.value)} placeholder="Nama penulis" />
                            </div>
                            <div>
                                <FormLabel>Email (opsional)</FormLabel>
                                <Input value={penulisEmail} onChange={(e) => setPenulisEmail(e.target.value)} placeholder="email@contoh.com" />
                            </div>
                            <p className="text-xs text-muted-foreground">Kosongkan nama untuk menghapus (soft-delete) penulis.</p>
                        </div>
                    </div>
                    ...
                    <div className="border rounded-lg p-4">
                        ...
                        <div className="space-y-3">
                            <div>
                                <FormLabel>Nama</FormLabel>
                                <Input value={editorName} onChange={(e) => setEditorName(e.target.value)} placeholder="Nama editor" />
                            </div>
                            <div>
                                <FormLabel>Email (opsional)</FormLabel>
                                <Input value={editorEmail} onChange={(e) => setEditorEmail(e.target.value)} placeholder="email@contoh.com" />
                            </div>
                            <p className="text-xs text-muted-foreground">Kosongkan nama untuk menghapus (soft-delete) editor.</p>
                        </div>
                    </div>
                </div>
                {/* === /Contributors (NEW) === */}
...
//                         <FormItem className='flex w-full flex-col'>
//                             <FormLabel className='text-base-semibold text-light-2'>
//                                 Tags / Keywords (Seperated by comma ,)
//                             </FormLabel>
//                             <FormControl>
//                                 <Input
//                                     type='text'
