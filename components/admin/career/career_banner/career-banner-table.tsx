...
import {deleteCareerBanner, fetchCareerBanners} from "@/lib/actions/admin/career_banner.action";
import AddEditCareerBanner from "@/components/admin/career/career_banner/edit-career-banner";
...
function CareerBannerTable() {

    const [achievements, setAchievements] = useState<CareerBanner[]>()
    async function getAchievements() {
        const achievements = await fetchCareerBanners()
    ...
    const [open, setOpen] = useState<{banner : CareerBanner | null, isOpen : boolean}>({banner: null, isOpen:false});
    const [deleteLoading, setDeleteLoading] = useState(false);
    const [createBannerOpen, setCreateBannerOpen] = useState<{banner : CareerBanner | null, isOpen : boolean}>({banner: null, isOpen: false})

    const handleDelete = async (id: string) => {
        try {
            setDeleteLoading(true);
            await deleteCareerBanner({id: id});
    ...
    return (
            <div className="flex flex-col">
                ...
                <Dialog open={createBannerOpen.isOpen} onOpenChange={(isOpen) => setCreateBannerOpen(prevState => {
                    ...
                    }} variant="outline" className="w-fit ml-8"><PlusIcon className="w-4 h-4"/> Add Banner</Button>
                    <DialogContent className="w-8">
                        <DialogHeader>
                            <DialogTitle>{"Add Banner"}</DialogTitle>
                        </DialogHeader>
                        <DialogBody className="overflow-y-auto max-h-[420px]">
                            <AddEditCareerBanner achievement={createBannerOpen.banner == null ? undefined : createBannerOpen.banner} onNeedRefresh={() => {
                        ...
                        <TableBody>
                            {achievements?.map((achievement) => (
                                <TableRow key={achievement.id}>
                                    ...
                                    <TableCell><Image src={achievement.image} alt="" width={50} height={50} /></TableCell>
                                    <TableCell>
...
export default CareerBannerTable;